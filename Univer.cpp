...
/*
	...
	Вариант 4. 
	Распечатать всех студентов в порядке 
	убывания 5 в одном, нескольких или 
	всех семестрах, которые выбираются 
	по желанию пользователя.

*/
void Univer::var4query(int sc, int* sarr) {
	// массив кол-ва пятерок студентов
	...
	if (sc == 0) {
		...
		for (int i = 0; i < N;i++) {
			...
			for (int j = 0;j < tmpS->_zach->sc;j++) {
				for (int k = 0;k < tmpS->_zach->_sems[j]->sc;k++) {
					mar[tmpS->_zach->_sems[j]->_subjects[k]->mark]++; // собираем все оценки студента
																	  // за все семестры в массив
				}
			}

			fiveArr[i] = mar[5]; // колво пятерок
		...
		for (int i = 0; i < N;i++) {
			...
			Student* tmpS = &((*_students)[i]);
			for (int s = 0;s < sc;s++) {
				for (int k = 0;k < tmpS->_zach->_sems[sarr[s]]->sc;k++) {
					mar[tmpS->_zach->_sems[sarr[s]]->_subjects[k]->mark]++; // собираем все оценки студента
																	  // за все семестры в массив
				}
			}
			fiveArr[i] = mar[5]; // колво пятерок
	...
	// сортировка пузырьком массива кол-ва пятерок
	// при этом параллельно с элементами массива оценок
	// двигаем и индексы студентов во втором массиве, чтобы
	// знать, у какого студента такая оценк
	int t, ti; // переменные для хранения временных значений
	for (int i = 0; i < N - 1; i++) {
		for (int j = 0; j < N - i - 1; j++) {
			if (fiveArr[j] > fiveArr[j + 1]) {
				t = fiveArr[j];
				ti = inds[j];

				fiveArr[j] = fiveArr[j + 1];
				inds[j] = inds[j + 1];
	...
	cout << "Отсортированный в порядке убывания массив: " << endl;
	for (int i = N - 1; i >= 0;i--) {
