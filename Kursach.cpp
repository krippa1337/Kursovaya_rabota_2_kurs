...
menu_n: 
	...
	cout << "5. Показать выборку (вариант 4)" << endl;
	cout << "0. Выход" << endl;
	cout << "> ";
	...
	case 5: {
		...
		string ssecount = "";
	scentr:
		cout << "Введите кол-во сессий, для которых нужно произвести выборку (0=все сессии) > ";
		getline(cin, ssecount);
		if (!isValidInt(ssecount) || stoi(ssecount) > 12 || stoi(ssecount) < 0) {
			cls;
			cout << INVINP << endl;
			goto scentr;
		}
		...
		string item = "";
		int* arr = new int[stoi(ssecount)];
		if (stoi(ssecount)) {
			for (int i = 0;i < stoi(ssecount);i++) {
				cout << "Введите номер сессии [" << (i + 1) << "] > ";
				getline(cin, item);
				if (!isValidInt(item) || stoi(item) > 12 || stoi(item) < 1) {
					i--; // если номер семестра введен неверно - повторить итерацию
					cout << INVINP << endl;
				}
				else {
					arr[i] = stoi(item) - 1; // номер сессии в массиве идёт от нуля
				}
			}
		}


		u->var4query(stoi(ssecount), arr); // передать в функцию выборки
		// кол-во сессий и адрес начала массива сессий
